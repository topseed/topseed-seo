...
{	
	BLX = require('../../../root/_js/BLX')
	BDS = require('../../../root/_js/BDS')
}*/
...
function ListBusiness() {// 'closure|module'-iso.
	...
	var SimpleBusiness = BLX.extend({
		...
		, ssrList: function(ssrdoc, listId, templateId) {
			sb.siteListDao.selectList().then(function(values) {
					var templateText = ssrdoc.getElementById(templateId).text
					var templateFunction = doT.template(templateText)
					ssrdoc.getElementById(listId).innerHTML = templateFunction({'array': values})
